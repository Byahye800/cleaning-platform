...
**Title:** Client-facing issue reporting on jobs, plus dashboard actor-mislabeling hotfix

**Description:** Clients had no way to report a problem on a job or follow up with comments; all issue reporting was staff-only. Separately, an in-cycle regression was discovered on the admin Dashboard: the "Recent Activity" feed's actor-resolution logic did not know about client-originated actors, so activity generated by a client (e.g. reporting an issue) rendered the actor name as "Admin" instead of the client's real name.

**Root Cause:** Primary feature: no schema, RLS policy, or RPC surface existed for clients to write issues/comments against their own jobs; role-check constraints on `issues.reported_by_role` and `issue_comments.author_role` only admitted `'staff'`/`'cleaner'` values. Hotfix: the dashboard's Recent Activity actor-name resolver (`src/app/admin/page.tsx`) only ever looked up `actor_id` against the cleaners table before falling back to `'Admin'`; it was never extended to also check the clients table, unlike the job-detail page's resolver (`admin/jobs/[id]/page.tsx`, commit `eb06b0f`), which already had the correct cleaner→client→Admin fallback chain.

**Affected Area:** `supabase/0032_client_issue_reporting.sql`; `src/app/admin/page.tsx` (Dashboard Recent Activity widget); client-facing issue reporting UI/RPC surface.

**Discovered:** 2026-07-21 (dashboard mislabeling found during in-cycle regression review of CLIENT-ISSUES-001).

**Status:** RESOLVED — LOCKED (2026-07-21)

**Priority:** P2 (feature gap + user-facing display defect; no data integrity or security impact).

**Resolution:** Migration `0032_client_issue_reporting.sql` (commit `0cf4e54`) widens `issues_reported_by_role_check` and `issue_comments_author_role_check` to admit `'client'`; adds RLS SELECT policies "Clients read own job issues" and "Clients read own job issue comments" scoped via `jobs`/`clients`/`user_roles` ownership join; adds `client_report_issue(p_job_id, p_description)` and `client_add_issue_comment(p_issue_id, p_body)` as `SECURITY DEFINER` RPCs granted to `authenticated` only; corrects `notify_on_new_issue_comment()` to branch three ways by author role. Client-facing UI and integration work landed in commits `a127d81` and `b94dc93`. Dashboard hotfix (commit `7d986c5`) mirrors the existing `admin/jobs/[id]/page.tsx` pattern: added a `clientsForFeedRes` query (`supabase.from('clients').select('user_id, name').in('user_id', actorIds)`) and a `clientNameByUserId` map, changing actor-name resolution to `cleanerNameByUserId.get(row.actor_id) ?? clientNameByUserId.get(row.actor_id) ?? 'Admin'`.

**Verification:** Root-cause confirmed before any modification. Client issue-reporting flow live E2E-verified on staging (report + comment, RLS-scoped visibility, notification branch). Dashboard hotfix repair remained tightly scoped to `src/app/admin/page.tsx` only, with no unnecessary architectural refactor. Full sequence completed: root-cause confirmation, minimal implementation, TypeScript, ESLint, production build, deployment, live verification, regression verification. Live verification confirmed Client A and Client B activity now render with correct client identity (never "Admin"); admin- and Stripe-originated activity rows unaffected; all other Dashboard widgets rendering correctly; core CLIENT-ISSUES-001 job-detail flows unchanged.

**Hotfix (in-cycle repair, pre-LOCK):** Disclosed limitation: no live cleaner-originated activity was available on staging during this verification pass, so the cleaner-name resolution branch could not be exercised live end-to-end. Accepted because the cleaner resolution branch remained byte-identical to its pre-hotfix form, no behavioral change was introduced to that path, and the limitation is explicitly disclosed here rather than hidden. This repair was independently reviewed; the engineering approach, root-cause identification, and verification sequence were all approved prior to LOCK.
...
**Status:** RESOLVED — LOCKED (2026-07-22)
...
**LOCK closure (2026-07-22):** independently reviewed and approved. BUILD implementation, migration design, scheduling algorithm documentation, concurrency documentation, rollback strategy, regression verification, and documentation updates all accepted. Recorded as RESOLVED and LOCKED. No further code changes are authorised for SCHEDULE-INTEGRITY-001 unless a new verified production defect is formally opened as its own engineering cycle.
