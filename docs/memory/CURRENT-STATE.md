...
## ADMIN-CLEANERS-001 (2026-07-21) — LOCKED, separate track

Not part of the Stage 1–2.5 / Phase 0–7 tracks below. Admin create/edit-cleaner functionality (previously broken — the write path had been removed and never rebuilt behind Route Handlers) is now restored, live E2E-verified against staging, and **LOCKED** per `docs/ENGINEERING-PROTOCOL.md`'s Production Engineering Confirmation Cycle. Delivered via `src/app/api/admin/cleaners/route.ts` (commit `ddcc813`), `src/app/api/admin/cleaners/[id]/route.ts` (commit `89c1571`), `src/app/admin/cleaners/page.tsx` (commit `c3ed0bd`), and `supabase/0031_admin_cleaner_write_rpcs.sql` (commit `3d67841`). Per standing instruction, this module is now the production baseline for admin cleaner create/edit — any further change requires a new DESIGN phase and a full confirmation cycle before another LOCK is requested. Full detail: `docs/KNOWN-ISSUES-REGISTER.md` (`ADMIN-CLEANERS-001`), `docs/memory/ACTIVE-WORK.md`, `docs/SESSION-LOG.md` (2026-07-21 entry).
