...
| Checkpoint 4 Part A (public signup disabled, min password length 6->8) | **Dashboard verified** | Pre-change and post-change fresh screenshots on the confirmed staging project (`jwdfzgibrijcyypibhjw`); save-safety gate performed before each save comparing every visible adjacent field; "Successfully updated settings" confirmed on signup-toggle save; minimum-password-length value independently re-verified as persisted by reopening the panel after navigation; full page-text extraction confirmed every provider state (Email enabled; Phone/SAML/Web3 Wallet/all OAuth providers disabled) with certainty before the change. No live Auth call (signup, invite, or password reset) was tested — that remains DB/route/browser-verification scope for later checkpoints. 2026-07-14 |

| Checkpoint 4 Part B dependency review (2026-07-14) | **Repository verified (read-only)** | Fresh clone confirmed HEAD matched the last committed state before review; grep/read of every redirect-constructing call site (`invitations/invite/route.ts`, `invitations/resend/route.ts`, `admin/login/page.tsx`, `onboarding/page.tsx`, `reset-password/page.tsx`, `proxy.ts`) confirmed the Site URL/Redirect URL dependency chain described above. No Supabase or Vercel setting was changed; Checkpoint 4 Part B configuration remains BLOCKED pending a stable, working staging deployment. |

| Checkpoint 4 Part B configuration (2026-07-15) | **Dashboard + deployment verified** | Site URL and both code-proven Redirect URLs set on the confirmed staging project (`jwdfzgibrijcyypibhjw`) and independently re-verified persisted via fresh page reload; `NEXT_PUBLIC_APP_URL` added to the staging Vercel project scoped to Production only, the two existing Supabase env vars left unchanged; one redeploy triggered (deployment `Bqdw3YzcZSJxcjYLphqN8m73jC1M`, commit `f494e6b`) reached Ready in 52s with no build errors; root URL loads, redirects to `/admin/login`, renders correctly, zero browser console messages on two fresh loads. No functional auth test performed. Production not touched. |

| Staging admin bootstrap pre-execution review (2026-07-15) | **Code + live schema verified (read-only)** | Re-inspected `requireAdmin()`, `reserve_account_invitation`'s admin-existence check, live `user_roles` schema/constraints/RLS/triggers via direct SQL introspection, live row counts (all 0 across `auth.users`/`user_roles`/`cleaners`/`clients`/`account_invitations`), and the repository's own historical bootstrap precedent (`supabase/0002_admin_role_seed.sql`, never applied to staging). Flagged two live schema gaps as open risks: `user_roles.user_id` has no `NOT NULL`/FK constraint, `user_roles.role` has no `CHECK` constraint. Recommendation: APPROVE BOOTSTRAP. No account created, no Supabase modified. |

| Staging first-admin bootstrap execution (2026-07-15) | **Live-verified end-to-end** | Owner-authorized controlled execution. Pre-execution counts confirmed 0 across all 5 relevant tables (a transient stale dashboard "10 users (estimated)" figure was observed and investigated — resolved to "No users in your project"; SQL ground truth was 0 throughout). One Auth user created (email entered by assistant, password entered directly by owner and never seen/typed/stored by the assistant). One `user_roles` admin row inserted via a guarded transaction (pre-checks for existing Auth user and empty `user_roles`, single literal insert, post-check verification, all inside one `begin`/`commit`) — query text visually verified against intent before execution, transaction succeeded with no exception. Linkage confirmed exact via join query (`user_roles.user_id` = `auth.users.id`, `role` = `'admin'`). Login test passed (reached `/admin`, dashboard rendered; a pre-existing unrelated `job_billing` schema-cache error was observed and is not a bootstrap-caused issue). Session-persistence test passed (refresh + navigation to `/admin/clients` succeeded, view-only). Logout test passed (session ended, redirected to login). Route-protection test passed (post-logout refresh did not restore session; direct navigation to `/admin/clients` redirected to `/admin/login?next=...`). Post-test broad count re-verification confirmed no unrelated row was created in `cleaners`, `clients`, `account_invitations`, `jobs`, `attendance`, `payroll_events`, or `activity_log`. Production not accessed. No credential recorded anywhere. |
| Checkpoint 6 Phase A — real working Vercel deployment (2026-07-14) | **Deployment verified** | Following the Stripe module-scope build-blocker code correction, a real deployment succeeded, producing `https://cleaning-platform-staging.vercel.app`. Confirmed reachable and rendering correctly when subsequently used as the Checkpoint 4 Part B Site URL/Redirect URL target (2026-07-15) and again during the staging admin bootstrap login test. This supersedes the earlier accidental failed deployment recorded in the INCIDENT entry below, which never served any route. |
| Checkpoint 5 — staging custom SMTP via Resend (2026-07-17) | **Route/delivery verified** | Supabase staging project's Authentication -> SMTP Settings configured (host `smtp.resend.com`, port 465, sender `onboarding@resend.dev`, a new sending-scoped Resend API key). One real invitation email sent via the app's (at the time API-only) invite route to the owner's own inbox (`bakar.yahye@gmail.com`) and confirmed received. Known limitation, not yet resolved: Resend's free-tier test sender only delivers to the Resend account owner's own address until a domain is verified. |
| STAGING-003 — `job_billing`/`cleaner_pay_rates` migration-capture gap (2026-07-18) | **DB verified, resolved** | Root-caused across five independent review passes (original, supplementary, design, adversarial, final pre-write hardening) plus a 60-check live test battery (schema, RLS, triggers, idempotency, app/build). Migration `0029_job_billing_and_cleaner_pay_rates_schema.sql` applied to staging only; captures tables/function that already existed live in production but were never in a repository migration file. Full detail in `KNOWN-ISSUES-REGISTER.md`, `STAGING-003`. |
| Admin Invite UI built + deployed; real invite-send test (2026-07-18) | **Browser/route verified (partial — delivery not achieved)** | `InviteForm.tsx` built and wired into the Cleaners and Clients admin pages, deployed to staging (commit `bc86d69`, after an initial bad paste at `5e45f3b` was caught and corrected). A real invite sent from the deployed page to `bakar.yahye+cleaner@gmail.com` correctly exercised `reserve_account_invitation` and the failure-compensation path (invitation marked failed, `activity_log` audit row written) when `inviteUserByEmail` failed — traced to the Checkpoint 5 Resend free-tier delivery restriction above, not a code defect. Email itself was not delivered. |

**Explicitly not claimed here:** no functional/behavioral testing of staging beyond the narrow STAGING-002 trigger-path test above has been performed, and Checkpoint 4 Part A's dashboard-verified settings have not been exercised by any live signup/password-reset call. Checkpoint 4 Part B's configuration is live-verified. The staging first-admin bootstrap is live-verified end-to-end (login/session/logout/route-protection only). `reserve_account_invitation` has now been exercised once for real (2026-07-18) via the Invite UI, with the reservation and failure-compensation paths confirmed correct — but no invitation has actually been delivered end-to-end, no password-reset or onboarding flow has been exercised, and no cleaner or client account exists yet. That remains Checkpoint 6 Phase B/Checkpoint 8's scope, currently blocked on the Resend domain-verification issue (see `KNOWN-ISSUES-REGISTER.md`, `STAGING-003` is unrelated and already resolved). Checkpoint 4 overall: **COMPLETE** (Part A and Part B both passed). Checkpoint 5: **COMPLETE** (with the known delivery limitation above). Checkpoint 6 Phase A: **COMPLETE**. STAGING-001 was not touched.
...
| `NEXT_PUBLIC_APP_URL` | Vercel deployment-environment dependency used by the invitation API routes when generating redirect URLs — not confirmed as a build-time dependency (prior build evidence showed the build compiled without it). Its exact deployment-time/runtime behaviour, and whether a redeployment is required after changing it in Vercel to guarantee the deployed application uses the updated value, must be verified during Checkpoint 6 rather than assumed. |
...
**Stripe build-blocker code correction (2026-07-14, code repository verified):** `src/app/