...
**Explicitly not claimed here:** no functional/behavioral testing of staging beyond the narrow STAGING-002 trigger-path test above has been performed, and Checkpoint 4 Part A's dashboard-verified settings have not been exercised by any live signup/invite/password-reset call. Checkpoint 4 overall is partially complete (Part A only); Part B (URL-dependent) dependency review passed, but configuration remains blocked and not started. STAGING-001 was not touched.
...
**Stripe build-blocker code correction (2026-07-14, code repository verified):** `src/app/api/stripe/send-invoice/route.ts`'s module-scope `new Stripe(process.env.STRIPE_SECRET_KEY!)` — the confirmed cause of the "Proven required" row above — was relocated to handler scope (inside the existing `try` block, immediately before the first Stripe call), matching the pattern already used in `webhook/route.ts`. Verified via `git diff` (single file, 1 insertion / 2 deletions, no unrelated changes) and `npx tsc --noEmit` (zero errors). **No deployment, no Vercel environment-variable change, and no Supabase change was made.** By code inspection this eliminates the specific previously observed build-time failure path; it does not confirm the build will succeed end-to-end, since no build has ever progressed past this point to reveal whether further blockers exist (e.g. around the two Supabase `NEXT_PUBLIC_*` variables). Checkpoint 6 remains BLOCKED.
