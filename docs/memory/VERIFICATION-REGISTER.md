...
**Explicitly not claimed here:** no functional/behavioral testing of staging beyond the narrow STAGING-002 trigger-path test above has been performed, and Checkpoint 4 Part A's dashboard-verified settings have not been exercised by any live signup/invite/password-reset call. Checkpoint 4 Part B's configuration is now live-verified, but no invitation, password-reset, or onboarding flow has actually been exercised end-to-end — that remains Checkpoint 6 Phase B scope, currently blocked on the absence of any staging test account. Checkpoint 4 overall: **COMPLETE** (Part A and Part B both passed). STAGING-001 was not touched.
